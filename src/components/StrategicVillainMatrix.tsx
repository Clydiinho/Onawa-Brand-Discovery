    ...
    <div className="flex flex-col gap-8 animate-fadeIn">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2 text-xs font-mono font-black text-[#C1FF00] uppercase tracking-widest">
          <Skull className="w-4 h-4 text-[#C1FF00]" />
      ...
      <div className="p-4 bg-slate-950/90 rounded-2xl border border-[#00FFC2]/50 shadow-xl flex items-start gap-3 relative group">
        <div className="p-2.5 bg-[#00FFC2]/10 border border-[#00FFC2]/40 rounded-xl text-[#00FFC2] shrink-0">
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            ...
            <div className="relative inline-block">
              ...
              {showTooltip && (
                <div className="absolute left-0 bottom-full mb-2 w-64 p-2.5 bg-slate-900 border border-[#00FFC2] rounded-xl text-[11px] text-slate-200 z-50 shadow-2xl">
      ...
      <div className="p-6 bg-slate-950/90 rounded-2xl border border-white/20 shadow-xl flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <label className="text-xs font-extrabold text-[#C1FF00] uppercase tracking-wider flex items-center gap-2">
            <Crosshair className="w-4 h-4" />
            <span>Define Your Strategic Enemy <span className="text-[#FF002B]">*</span></span>
          </label>
          <span className="text-[10px] font-mono text-slate-400">The Problem You Defeat</span>
        </div>

        {/* Preset Chips */}
        <div className="flex flex-col gap-2">
          <span className="text-[11px] font-bold text-slate-300">Quick Strategic Villain Archetypes:</span>
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all flex items-center gap-1.5 border ${
                    isSelected
                      ? "bg-[#C1FF00] text-slate-950 border-[#C1FF00] font-black shadow-md shadow-[#C1FF00]/20"
                      : "bg-slate-900/90 text-slate-300 border-slate-700 hover:border-[#C1FF00]/60 hover:text-white"
                  }`}
                >
                  {isSelected ? <Check className="w-3.5 h-3.5 stroke-[3]" /> : <Zap className="w-3.5 h-3.5 text-[#C1FF00]" />}
                  <span>{preset}</span>
                </button>
              );
            })}
      ...
      <div className="p-6 bg-slate-950/90 rounded-2xl border border-white/20 shadow-xl flex flex-col gap-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-4">
          ...
          <div className="px-3.5 py-1.5 bg-slate-900 border border-[#00FFC2]/40 rounded-xl text-xs font-mono flex items-center gap-2">
        ...
        <div className="relative w-full aspect-square max-w-xl mx-auto bg-slate-950 rounded-2xl border-2 border-slate-800 p-2 overflow-hidden shadow-2xl select-none">
          {/* Axis Labels Overlay */}
          {/* Top Label: PROGRESSIVE */}
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 pointer-events-none px-3 py-1 bg-slate-900/90 border border-[#00FFC2]/50 text-[#00FFC2] font-mono font-black text-[10px] tracking-widest uppercase rounded-full shadow-lg">
            ▲ PROGRESSIVE (NEXT-GEN)
          </div>

          {/* Bottom Label: TRADITIONAL */}
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 pointer-events-none px-3 py-1 bg-slate-900/90 border border-slate-700 text-slate-400 font-mono font-bold text-[10px] tracking-widest uppercase rounded-full">
            ▼ TRADITIONAL (LEGACY)
          </div>

          {/* Left Label: CORPORATE */}
          <div className="absolute left-3 top-1/2 -translate-y-1/2 z-20 pointer-events-none px-2 py-1 bg-slate-900/90 border border-slate-700 text-slate-400 font-mono font-bold text-[10px] tracking-widest uppercase rounded-full -rotate-90 origin-center">
            ◀ CORPORATE
          </div>

          {/* Right Label: DISRUPTIVE */}
          <div className="absolute right-3 top-1/2 -translate-y-1/2 z-20 pointer-events-none px-2 py-1 bg-slate-900/90 border border-[#C1FF00]/60 text-[#C1FF00] font-mono font-black text-[10px] tracking-widest uppercase rounded-full rotate-90 origin-center shadow-lg">
            DISRUPTIVE ▶
          </div>
          ...
          <div className="absolute inset-0 grid grid-cols-2 grid-rows-2 pointer-events-none p-8 text-[11px] font-mono font-extrabold uppercase">
            ...
            <div className="p-4 text-[#C1FF00]/80 bg-[#C1FF00]/5 border-2 border-dashed border-[#C1FF00]/30 rounded-xl flex flex-col justify-start items-end text-right animate-pulse">
              <span className="bg-[#C1FF00] text-slate-950 px-2 py-0.5 rounded font-black text-[9px] mb-1">
                ★ TARGET ZONE
              </span>
              <span>BLUE OCEAN GAP</span>
              <span className="text-[9px] text-[#00FFC2]">High Margin Opportunity</span>
            </div>
          ...
          >
            ...
            {/* Center Axes Lines */}
            <line x1="200" y1="20" x2="200" y2="380" stroke="#00FFC2" strokeWidth="2" strokeDasharray="4 4" opacity="0.6" />
            <line x1="20" y1="200" x2="380" y2="200" stroke="#00FFC2" strokeWidth="2" strokeDasharray="4 4" opacity="0.6" />

            {/* Center Origin Dot */}
            <circle cx="200" cy="200" r="4" fill="#00FFC2" opacity="0.8" />

            {/* User Target Pin Marker */}
            <g transform={`translate(${svgX}, ${svgY})`} filter="url(#glow)">
              {/* Pulsing Outer Rings */}
              <circle r="22" fill="none" stroke="#C1FF00" strokeWidth="1.5" opacity="0.4" className="animate-ping" />
              <circle r="14" fill="rgba(193, 255, 0, 0.25)" stroke="#C1FF00" strokeWidth="2" />

              {/* Center Icon Graphic */}
              <circle r="8" fill="#C1FF00" />
        ...
        <div className="p-4 bg-slate-900 rounded-xl border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2">
            <MapPin className="w-4 h-4 text-[#C1FF00]" />
            <span className="text-slate-300 font-bold">Identified Market Position:</span>
            <span className="font-black text-[#C1FF00] font-mono">{matrix.quadrant}</span>
          </div>

          <div className="text-[11px] text-slate-400 flex items-center gap-1">
            <Info className="w-3.5 h-3.5 text-[#00FFC2]" />
