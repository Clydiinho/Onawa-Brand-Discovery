...
export const InteractiveMoodBoard: React.FC<InteractiveMoodBoardProps> = ({
  ...
  const [selectedHex, setSelectedHex] = useState("#C1FF00");
  ...
  useEffect(() => {
    ...
    if (moodBoard.elements && moodBoard.elements.length > 0) {
      moodBoard.elements.forEach((elem) => {
        if (elem.type === "color") {
          ...
          const textObj = new fabric.Text(elem.content, {
            ...
            fontSize: 32,
            fill: elem.content.startsWith("#") ? elem.content : "#C1FF00",
            angle: elem.angle || 0,
            scaleX: elem.scaleX || 1,
            scaleY: elem.scaleY || 1,
          });
      ...
      const swatchText = new fabric.Text("#00FFC2\nCYBER CYAN", {
        fontSize: 12,
        fontFamily: "monospace",
        fill: "#020617",
        left: 65,
  ...
  const handleAddColorSwatch = (hex: string) => {
    ...
    const label = new fabric.Text(hex.toUpperCase(), {
      fontSize: 13,
      fontFamily: "monospace",
      fill: hex === "#F8FAFC" || hex === "#C1FF00" || hex === "#00FFC2" ? "#020617" : "#ffffff",
      left: 115,
      top: 155,
      fontWeight: "bold",
    });
    ...
    <div className="flex flex-col gap-8 animate-fadeIn">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2 text-xs font-mono font-black text-[#C1FF00] uppercase tracking-widest">
          <Palette className="w-4 h-4 text-[#C1FF00]" />
      ...
      <div className="p-4 bg-slate-950/90 rounded-2xl border border-[#C1FF00]/50 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-start gap-3">
          <div className="p-2.5 bg-[#C1FF00]/10 border border-[#C1FF00]/40 rounded-xl text-[#C1FF00] shrink-0">
            <Sparkles className="w-5 h-5 stroke-[2.5]" />
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-xs font-mono font-bold text-[#C1FF00] uppercase tracking-wider">
              Clyde’s Perspective:
            </span>
        ...
        <div className="text-[11px] font-mono text-slate-400 shrink-0 border-l md:border-l border-slate-800 pl-3">
      ...
      <div className="p-6 bg-slate-950/90 rounded-2xl border border-white/20 shadow-xl flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 border-b border-slate-800 pb-6">
          ...
          <div className="flex flex-col gap-2.5 p-4 bg-slate-900/90 rounded-xl border border-slate-800">
            <span className="text-xs font-extrabold text-[#C1FF00] uppercase tracking-wider flex items-center gap-1.5">
            ...
            <label className="cursor-pointer py-2 px-3 bg-slate-950 hover:bg-slate-800 border border-slate-700 hover:border-[#C1FF00] text-slate-200 text-xs font-bold rounded-xl flex items-center justify-center gap-2 transition-all">
              <Upload className="w-3.5 h-3.5 text-[#C1FF00]" />
          ...
          <div className="flex flex-col gap-2.5 p-4 bg-slate-900/90 rounded-xl border border-slate-800">
            <span className="text-xs font-extrabold text-[#00FFC2] uppercase tracking-wider flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                value={selectedHex}
                onChange={(e) => setSelectedHex(e.target.value)}
                className="w-24 px-2 py-1 bg-slate-950 border border-slate-700 text-white font-mono text-xs rounded-lg uppercase"
              />
              <button
                type="button"
                onClick={() => handleAddColorSwatch(selectedHex)}
                className="px-3 py-1 bg-[#00FFC2] hover:bg-[#00d6a3] text-slate-950 font-black text-xs rounded-lg transition-all"
              >
                + Pin
              </button>
            </div>
          ...
          <div className="flex flex-col gap-2.5 p-4 bg-slate-900/90 rounded-xl border border-slate-800">
            ...
            <input
              ...
              className="w-full px-2.5 py-1.5 bg-slate-950 border border-slate-700 text-white text-xs rounded-lg"
            />
            ...
            <div className="flex items-center gap-2">
              <select
                ...
                className="w-full px-2 py-1 bg-slate-950 border border-slate-700 text-slate-200 text-xs rounded-lg"
              >
              ...
              <button
                ...
                className="px-3 py-1 bg-purple-500 hover:bg-purple-600 text-white font-bold text-xs rounded-lg shrink-0 transition-all"
              >
                + Add
              </button>
            </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-900/60 p-3 rounded-xl border border-slate-800">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 border border-slate-700 text-slate-400 hover:text-white font-mono text-xs rounded-lg transition-all"
            >
              Clear Canvas
            </button>
          </div>
        ...
        <div
          ref={containerRef}
          className="relative w-full rounded-2xl border-2 border-slate-800 overflow-hidden shadow-2xl bg-slate-950 min-h-[550px]"
        >
